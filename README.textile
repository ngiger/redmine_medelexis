...
bundle exec rake redmine:plugins NAME=redmine_questions
bundle exec rake redmine:plugins NAME=redmine_silencer
...
bc. bundle exec rails console RAILS_ENV=development
Project.all.first
...
bc. RAILS_ENV=development bundle exec script/rails runner 'user = User.find(:first, :conditions => {:admin => true, :name => 'admin'}) ; \
...
@bundle exec ruby script/rails server webrick RAILS_ENV=development --port=30001@
...
bc. bundle exec rake db:migrate RAILS_ENV=test
bundle exec rake redmine:plugins NAME=redmine_contacts           RAILS_ENV=test
bundle exec rake redmine:plugins NAME=redmine_contacts_helpdesk  RAILS_ENV=test
bundle exec rake redmine:plugins NAME=redmine_products           RAILS_ENV=test
bundle exec rake redmine:plugins NAME=redmine_contacts_invoices  RAILS_ENV=test

Run tests

* @bundle exec rake redmine:plugins:test NAME=redmine_medelexis RAILS_ENV=test@ # runs all tests
* @bundle exec ruby script/rails runner plugins/redmine_medelexis/test/functional/license_test.rb  RAILS_ENV=test@ # runs a single test
